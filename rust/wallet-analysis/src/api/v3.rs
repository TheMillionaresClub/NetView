...
use crate::types::{
    domain::DnsRecord,
    identity::WalletState,
    nft::NftItem,
    tokens::JettonBalance,
    transactions::{TransactionPage, TxSummary},
};
...
pub async fn get_wallet_states(
    ...
    let url = format!("{}/walletStates", client.base_url_v3);
...
pub async fn get_jetton_wallets(
    ...
    let url = format!("{}/jetton/wallets", client.base_url_v3);
...
pub async fn get_nft_items(client: &TonClient, address: &str) -> Result<Vec<NftItem>> {
    let url = format!("{}/nft/items", client.base_url_v3);
...
pub async fn get_transactions_page(
    ...
    let url = format!("{}/transactions", client.base_url_v3);
...
pub async fn get_dns_records(client: &TonClient, address: &str) -> Result<Vec<DnsRecord>> {
    let url = format!("{}/dns/resolve", client.base_url_v3);
    let resp: DnsResolveResp = match client.get(&url, &[("address", address)]).await {
        Ok(v) => v,
        Err(e) if is_not_found(&e) => return Ok(vec![]),
        Err(e) => {
            // Some addresses have no domain; treat any API error as empty rather
            // than hard-failing the whole profile.
            let _ = e;
            return Ok(vec![]);
        }
    };
