use wallet_analysis::{analyze_wallet, api, client::Network, types::ActorKind, TonClient};
...
fn make_client() -> TonClient {
    // Optionally load .env for RPC_API_KEY
    let _ = dotenvy::dotenv();
    let api_key = std::env::var("RPC_API_KEY").ok();
    TonClient::new(Network::Testnet, api_key)
}
...
async fn test_jettons_returns_list() {
    ...
    // Just confirm it's a valid (possibly empty) vec.
    let _ = jettons.len();
}
...
async fn test_full_profile_human() {
    let client = make_client();
    let profile = analyze_wallet(&client, HUMAN_WALLET)
        .await
...
async fn test_smart_contract_classification() {
    let client = make_client();
    let profile = analyze_wallet(&client, CONTRACT_ADDR)
        .await
        .expect("analyze_wallet should succeed");
