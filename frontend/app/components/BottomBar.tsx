...
export default function BottomBar() {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/wallet-network?address=${encodeURIComponent(rawAddress)}&limit=50`
        );
  ...
  if (!restored || !connected) {
    return (
      <footer className={`fixed bottom-16 sm:bottom-0 left-0 sm:left-20 right-0 w-auto z-50 flex flex-col bg-[#0a1018] border-t border-[#1c2d42] transition-all duration-300 ${collapsed ? 'h-auto' : 'h-[30vh] sm:h-[42vh]'}`}>
        {/* collapse toggle */}
        <button
